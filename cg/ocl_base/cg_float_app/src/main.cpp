...
int main(int argc, char *argv[])
{
  ...
  size_t nz_int_size_in_bytes = sizeof(int)   * (NV);
  size_t nz_val_size_in_bytes = sizeof(float) * (NV);
  ...
  cl::Buffer buffer_col(context, CL_MEM_READ_ONLY, nz_int_size_in_bytes);
  cl::Buffer buffer_val(context, CL_MEM_READ_ONLY, nz_val_size_in_bytes);
  ...
  krnl.setArg(narg++, buffer_col);
  krnl.setArg(narg++, buffer_val);
  ...
  int * ptr_colptr  = (int *) q.enqueueMapBuffer
    (buffer_colptr, CL_TRUE, CL_MAP_WRITE, 0, colptr_size_in_bytes);
  int * ptr_col     = (int *) q.enqueueMapBuffer
    (buffer_col,    CL_TRUE, CL_MAP_WRITE, 0, nz_int_size_in_bytes);
  float * ptr_val   = (float *) q.enqueueMapBuffer
    (buffer_val,    CL_TRUE, CL_MAP_WRITE, 0, nz_val_size_in_bytes);
  ...
  q.enqueueMigrateMemObjects
    ({buffer_colptr, buffer_col, buffer_val, buffer_b},
     0 /* 0 means from host */);
